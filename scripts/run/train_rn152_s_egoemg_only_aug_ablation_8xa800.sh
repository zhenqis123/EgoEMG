#!/usr/bin/env bash
# Queue RN152-S no-augmentation then legacy-WL7790-augmentation fusion runs.
set -Eeuo pipefail
...
run_job() {
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 "$PYTHON_BIN" -m egoemg.train \
    ...
    "vision_resnet_checkpoint=${INPUT_ROOT}/vision_checkpoints/rn152.ckpt" \
...
cd "$REPO"
run_job fusion/fusion_rn152_s_egoemg_only_noaug_dense fusion_rn152_s_egoemg_only_noaug_dense_lr5e-4_300e
run_job fusion/fusion_rn152_s_egoemg_only_legacy_wl7790_aug_dense fusion_rn152_s_egoemg_only_legacy_wl7790_aug_dense_lr5e-4_300e
