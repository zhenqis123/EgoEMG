#!/usr/bin/env bash
# Current best last-block fusion recipe with one added term:
# 0.01 * mean(delta ** 2).
set -Eeuo pipefail

REPO=${EMG2POSE_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}
LOG_ROOT=${REPO}/logs/20260726
RUN_NAME=fusion_rn50_m_egoemg_only_wl12000_crossattn_lastblock_anchor_augbest_deltareg0p01_20e
...
CUDA_VISIBLE_DEVICES="$GPUS" python -m egoemg.train \
  experiment=fusion/fusion_rn50_m_egoemg_only_wl12000_crossattn_lastblock_anchor_augbest_deltareg0p01_20e \
