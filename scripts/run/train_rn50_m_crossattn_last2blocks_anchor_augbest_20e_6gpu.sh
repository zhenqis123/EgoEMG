#!/usr/bin/env bash
# RN50-M cross-attention fusion with the final two layer4 Bottlenecks
# trainable.  BN and the vision pose head remain frozen/eval.
set -Eeuo pipefail

REPO=${EMG2POSE_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}
LOG_ROOT=${REPO}/logs/20260726
RUN_NAME=fusion_rn50_m_egoemg_only_wl12000_crossattn_last2blocks_anchor_augbest_20e
...
CUDA_VISIBLE_DEVICES="$GPUS" python -m egoemg.train \
  experiment=fusion/fusion_rn50_m_egoemg_only_wl12000_crossattn_last2blocks_anchor_augbest_20e \
