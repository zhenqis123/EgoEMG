...
class MultiSessionWindowedEmgDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        # Fields:
        # emg: EMG signal window (16, T). sEMG-RD wrist band, 2 kHz.
        # joint_angles: hand joint angles (20, T), from motion-capture + IK.
        # label_valid_mask: per-sample validity mask (T). False where IK failed or NaN.
        # window_start_idx/window_end_idx: indices within session for this window.
        # session_idx: integer index into sessions table.
        # user: subject identifier string.
        # side: recording side (left/right).
        return {
            "emg": emg.T,
...
def SplitFilteredHdf5Dataset(
    data_dir: str | Path,
    allowed_splits: list[str],
    **kwargs,
) -> MultiSessionWindowedEmgDataset:
    """Factory that scans data_dir for .hdf5 files, filters by split, and
    returns a MultiSessionWindowedEmgDataset."""
    data_dir = Path(data_dir)
    allowed = {_normalize_split(s) for s in allowed_splits}
    ...
    for p in sorted(data_dir.glob("**/*.hdf5")):
        ...
        with h5py.File(p, "r") as f:
            split = _normalize_split(str(f["emg2pose"].attrs.get("split", "unknown")))
        if split in allowed:
            paths.append(p)

    print(f"SplitFilteredHdf5Dataset: {len(paths)} sessions for splits {allowed}")
    return MultiSessionWindowedEmgDataset(hdf5_paths=paths, **kwargs)
