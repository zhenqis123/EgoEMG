...
This repository contains or interfaces with third-party components. This file
is a release-facing index, not a substitute for the license text supplied with
each component.
...
| WiLoR weights/assets | Not bundled; configured through external paths in research workflows | Separate upstream terms; not part of this release. |
| PyPI/Conda dependencies | Declared in `setup.py` and `environment.yml` | Each dependency retains its own license. |

The distribution has mixed licensing. Do not treat a package-level metadata
field as permission to relicense all contained material as MIT. A formal
data/model release must complete a file-level provenance and license review
before adding assets or a single unified license assertion.
