...
def make_lightning_module(config: DictConfig):
    return EmgToVQVAELightningModule(
        model_conf=config.emg2vqvae.model,
        vqvae_conf=config.emg2vqvae.vqvae,
        vqvae_checkpoint=config.emg2vqvae.vqvae_checkpoint,
        optimizer_conf=config.optimizer,
        lr_scheduler_conf=config.lr_scheduler,
        gumbel_conf=config.emg2vqvae.get("gumbel"),
        loss_weights=config.emg2vqvae.get("loss_weights"),
        label_smoothing=config.emg2vqvae.get("label_smoothing", 0.05),
    )
...
def train(
    ...
    if config.checkpoint is not None:
        ...
        log.info("Instantiating EMG->VQ-VAE LightningModule")
