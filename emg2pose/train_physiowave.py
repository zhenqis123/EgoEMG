...
from emg2pose.lightning_physiowave import PhysioWaveTemporalLightningModule
...
def make_data_module(config: DictConfig):
    def _full_paths(root: str, dataset: ListConfig) -> list[Path]:
        sessions = dataset
        return [Path(root).expanduser().joinpath(f"{session}.hdf5") for session in sessions]
...
def make_lightning_module(config: DictConfig):
    return PhysioWaveTemporalLightningModule(
        model_conf=config.physiowave_module,
...
def train(
    ...
    if config.checkpoint is not None:
        ...
        module = PhysioWaveTemporalLightningModule.load_from_checkpoint(
            config.checkpoint,
            model_conf=config.physiowave_module,
            optimizer_conf=config.optimizer,
            lr_scheduler_conf=config.lr_scheduler,
        ...
        log.info(f"Instantiating LightningModule {PhysioWaveTemporalLightningModule}")
...
@hydra.main(config_path="../config", config_name="physiowave_base", version_base="1.1")
