...
from emg2pose.lightning_cldm import CLDMDiffusionModule
...
def make_lightning_module(config: DictConfig):
    return CLDMDiffusionModule(
        unet_conf=config.unet,
        emg_vae_conf=config.emg_vae,
        pose_vae_conf=config.pose_vae,
        emg_vae_checkpoint=config.emg_vae_checkpoint,
        pose_vae_checkpoint=config.pose_vae_checkpoint,
        optimizer_conf=config.optimizer,
        lr_scheduler_conf=config.lr_scheduler,
        num_timesteps=config.get("num_timesteps", 1000),
        beta_start=config.get("beta_start", 1e-4),
        beta_end=config.get("beta_end", 2e-2),
        use_vae_mu=config.get("use_vae_mu", False),
    )
...
def train(config: DictConfig, extra_callbacks: Sequence[Callable] | None = None):
    ...
    if config.checkpoint is not None:
        ...
        module = CLDMDiffusionModule.load_from_checkpoint(
            config.checkpoint,
            unet_conf=config.unet,
            emg_vae_conf=config.emg_vae,
            pose_vae_conf=config.pose_vae,
        ...
        log.info("Instantiating CLDM Diffusion LightningModule")
...
@hydra.main(config_path="../config", config_name="cldm_diffusion_base", version_base="1.1")
